...
export default function RangedInput({ min, max, step, unit, defaultValue, callback }: RangedInput) {
  ...
  return (
    <div className="w-full relative h-16">
      <input ref={inputRef} type="range" min={0} max={max} step={step} defaultValue={defaultValue} className="w-full slider appearance-none" onInput={inputHandler} />
  ...
  function updateSlider(value: number) {
    ...
    button.style.left = (value / (max / 100)) * 0.955 + '%'
    slider.style.width = (value / (max / 100)) * 0.955 + 1 + '%'
  }
}
